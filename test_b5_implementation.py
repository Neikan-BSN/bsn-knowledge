...
class MockAnalyticsService:
    ...
    async def get_student_progress(self, student_id: str) -> Any:
        ...
        return StudentProgressMetrics(
            ...
            risk_factors=["time_management"] if student_id == "struggling_student" else [],
...
async def test_adaptive_learning_engine_initialization():
    ...
    try:
        ...
        print_success("AdaptiveLearningEngine initialized successfully with B.4 integration")
        print_info(f"Engine components: learning_analytics, gap_analyzer, path_optimizer")
        print_info(f"Performance thresholds configured: {adaptive_engine.performance_thresholds}")
...
async def test_personalized_content_generation(adaptive_engine):
    ...
    try:
        ...
        assert all(hasattr(rec, 'personalization_score') for rec in recommendations), "Missing personalization scores"
        assert all(rec.personalization_score > 0 for rec in recommendations), "Invalid personalization scores"

        print_success(f"Generated {len(recommendations)} personalized recommendations")

        # Display sample recommendations
        for i, rec in enumerate(recommendations[:3]):
            print_info(f"Recommendation {i+1}: {rec.title} (Score: {rec.personalization_score:.2f}, Success Prob: {rec.success_probability:.2f})")
...
async def test_learning_path_optimization(adaptive_engine):
    ...
    try:
        ...
        print_info("Optimizing learning path with B.4 analytics and RAGnostic integration...")
        ...
        assert "performance_predictions" in path_result, "Missing performance predictions"
        ...
        for feature in expected_features:
            assert adaptive_features.get(feature, False), f"Missing adaptive feature: {feature}"

        print_success(f"Learning path optimized: {path_result['path_id']}")
        print_info(f"Success probability: {path_result['performance_predictions']['completion_probability']:.2f}")
        print_info(f"Adaptive features enabled: {len([f for f, enabled in adaptive_features.items() if enabled])}")
...
async def test_dynamic_difficulty_adjustment(adaptive_engine):
    ...
    try:
        ...
        test_scenarios = [
            {
                ...
                "competency_context": {"domain_progressions": {"clinical_judgment": {"current_score": 0.85}}}
            },
            {
                "name": "Low Performance",
                "recent_performance": {"scores": [0.5, 0.48, 0.52], "engagement": 0.6},
                "competency_context": {"domain_progressions": {"clinical_judgment": {"current_score": 0.5}}}
            },
            {
                "name": "Declining Performance",
                "recent_performance": {"scores": [0.8, 0.7, 0.6], "engagement": 0.65},
                "competency_context": {"domain_progressions": {"clinical_judgment": {"current_score": 0.65}}}
            }
        ]
        ...
        for scenario in test_scenarios:
            ...
            # Validate adjustment
            assert hasattr(difficulty_adjustment, 'recommended_difficulty'), "Missing recommended difficulty"
            assert hasattr(difficulty_adjustment, 'confidence_score'), "Missing confidence score"
            assert hasattr(difficulty_adjustment, 'adjustment_reason'), "Missing adjustment reason"
            assert 0 <= difficulty_adjustment.confidence_score <= 1, "Invalid confidence score"
...
async def test_realtime_path_adaptation(adaptive_engine):
    ...
    try:
        ...
        competency_changes = {
            \"person_centered_care\": -0.15,
            \"clinical_judgment\": -0.1
        }
        ...
        # Validate adaptation
        assert hasattr(adaptation_result, 'adaptations_made'), \"Missing adaptations made\"
        assert hasattr(adaptation_result, 'estimated_improvement'), \"Missing improvement estimate\"
        assert hasattr(adaptation_result, 'adaptation_confidence'), \"Missing adaptation confidence\"
        assert len(adaptation_result.adaptations_made) > 0, \"No adaptations identified\"

        print_success(f\"Real-time adaptation completed: {adaptation_result.adapted_path_id}\")
        print_info(f\"Adaptations made: {len(adaptation_result.adaptations_made)}\")
        print_info(f\"Estimated improvement: {adaptation_result.estimated_improvement:.2f}\")
        print_info(f\"Adaptation confidence: {adaptation_result.adaptation_confidence:.2f}\")
...
async def test_adaptive_study_plan_generation(adaptive_engine):
    ...
    try:
        ...
        # Validate study plan
        required_fields = [
            \"plan_id\", \"personalized_content\", \"learning_path\",
            \"weekly_schedule\", \"milestones\", \"assessment_schedule\",
            \"adaptive_features\", \"success_predictions\", \"tracking_metrics\"
        ]
        ...
        assert \"completion_probability\" in success_predictions, \"Missing completion probability\"
        assert \"competency_targets\" in success_predictions, \"Missing competency targets\"
        assert 0 <= success_predictions[\"completion_probability\"] <= 1, \"Invalid completion probability\"

        print_success(f\"Adaptive study plan generated: {study_plan['plan_id']}\")
        print_info(f\"Study duration: {study_duration_weeks} weeks, {weekly_time_budget} min/week\")
        print_info(f\"Personalized content items: {len(study_plan['personalized_content'])}\")
        ...
        print_info(f\"Completion probability: {success_predictions['completion_probability']:.2f}\")
...
async def test_b4_integration_validation(adaptive_engine):
    ...
    try:
        ...
        student_analysis = await adaptive_engine.learning_analytics.analyze_student_progress(student_id)

        # Validate B.4 analysis structure
        required_analysis_fields = [
            \"progress_metrics\", \"competency_progression\", \"knowledge_gaps\",
            \"learning_recommendations\", \"risk_assessment\"
        ]
...
async def test_performance_and_caching(adaptive_engine):
    ...
    try:
        ...
        recommendations1 = await adaptive_engine.generate_personalized_content(student_profile)
...
🎯 B.5 SUCCESS CRITERIA VALIDATION:\")
    success_criteria = [
        (\"Personalization algorithm implemented\", \"Personalized Content Generation\" in test_results[\"test_details\"] and test_results[\"test_details\"][\"Personalized Content Generation\"] == \"PASSED\"),
        (\"Dynamic difficulty adjustment working\", \"Dynamic Difficulty Adjustment\" in test_results[\"test_details\"] and test_results[\"test_details\"][\"Dynamic Difficulty Adjustment\"] == \"PASSED\"),
        (\"Learning path optimization functional\", \"Learning Path Optimization\" in test_results[\"test_details\"] and test_results[\"test_details\"][\"Learning Path Optimization\"] == \"PASSED\"),
        (\"Integration with RAGnostic graphs tested\", \"Learning Path Optimization\" in test_results[\"test_details\"] and test_results[\"test_details\"][\"Learning Path Optimization\"] == \"PASSED\"),
        (\"Performance metrics tracked\", \"Performance and Caching\" in test_results[\"test_details\"] and test_results[\"test_details\"][\"Performance and Caching\"] == \"PASSED\")
    ]
...
🎉 B.5 ADAPTIVE LEARNING ENGINE IMPLEMENTATION: ✅ COMPLETE\")
        print(f\"   All tests passed and success criteria met per REVISED_PHASE3_PLAN.md\")
    else:
        print(f\"\
⚠️  B.5 IMPLEMENTATION STATUS: NEEDS ATTENTION\")
        print(f\"   Failed tests: {test_results['failed_tests']}, Unmet criteria: {len(success_criteria) - criteria_met}\")

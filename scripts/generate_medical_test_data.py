...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/home/user01/projects/bsn_knowledge/logs/medical_data_generation.log"),
...
class NursingEducationDocument:
    ...
    def __post_init__(self):
        ...
        if self.medical_concepts:
            self.umls_accuracy = sum(concept.confidence_score for concept in self.medical_concepts) / len(self.medical_concepts)
        else:
    ...
    def __post_init__(self):
        ...
        if self.clinical_domains is None:
            self.clinical_domains = [
                "medical_surgical", "pediatrics", "maternity", "psychiatric", 
                "community_health", "critical_care", "emergency", "oncology"
            ]
    ...
    def _load_validated_umls_concepts(self) -> Dict[str, MedicalConcept]:
        ...
        return {
            ...
            "hypertension": MedicalConcept(
                ...
                nclex_categories=["Physiological Integrity", "Health Promotion and Maintenance"],
            ...
            "congestive_heart_failure": MedicalConcept(
                ...
                synonyms=["CHF", "Heart Failure", "Cardiac Failure", "Left Heart Failure"],
                category="cardiovascular",
            ...
            "pneumonia": MedicalConcept(
                ...
                synonyms=["Lung Infection", "Pneumonic Process", "Respiratory Infection"],
            ...
            "diabetes_mellitus": MedicalConcept(
                ...
                synonyms=["Diabetes", "DM", "Sugar Diabetes", "Type 1 Diabetes", "Type 2 Diabetes"],
                category="endocrine",
                confidence_score=0.999,
            ...
            "nosocomial_infection": MedicalConcept(
                ...
                synonyms=["Healthcare-Associated Infection", "HAI", "Hospital-Acquired Infection"],
            ...
            "major_depression": MedicalConcept(
                ...
                synonyms=["Major Depression", "Clinical Depression", "Unipolar Depression"],
        ...
        for term in terms:
            ...
            if normalized_term in self.umls_concepts:
                ...
                synthetic_concept = MedicalConcept(
                    term=term,
                    umls_cui=f"C{random.randint(1000000, 9999999):07d}",
...
class NursingContentGenerator:
    ...
    async def generate_document(self, subject_area: str, difficulty: int, target_audience: str) -> NursingEducationDocument:
        ...
        medical_terms = self._generate_medical_terms_for_topic(selected_topic, subject_area)
        validated_concepts = await self.umls_validator.validate_terminology(medical_terms)
        ...
        learning_objectives = self._generate_learning_objectives(selected_topic, difficulty)
        
        # Create clinical scenarios
        clinical_scenarios = self._generate_clinical_scenarios(selected_topic, subject_area)
    ...
    def _generate_medical_terms_for_topic(self, topic: str, subject_area: str) -> List[str]:
        """Generate relevant medical terminology for the topic."""
        # Map topics to relevant medical terms
        term_mappings = {
            "cardiovascular": ["hypertension", "myocardial_infarction", "congestive_heart_failure"],
    ...
    def _generate_comprehensive_content(self, topic: str, subject_area: str, 
                                      difficulty: int, concepts: List[MedicalConcept]) -> str:
        ...
        content_sections = [
            ...
            f"The underlying pathophysiology involves complex physiological processes that require "
            f"thorough nursing assessment and understanding. Key clinical manifestations include "
            f"signs and symptoms that nurses must recognize early for optimal patient outcomes.",
            
            # Assessment section
            f"\n\nNursing Assessment:\n"
            f"Comprehensive nursing assessment includes systematic evaluation of patient status, "
            f"identification of risk factors, and ongoing monitoring of clinical indicators. "
            f"Priority assessments focus on patient safety and early detection of complications.",
            
            # Interventions section
            f"\n\nNursing Interventions:\n"
            f"Evidence-based nursing interventions are designed to promote patient outcomes, "
            f"prevent complications, and support patient and family education. Interventions "
            f"should be individualized based on patient needs and clinical condition.",
            
            # Patient Education section
            f"\n\nPatient and Family Education:\n"
            f"Education is a critical component of nursing care, focusing on health promotion, "
            f"disease prevention, and self-care management. Teaching should be culturally "
            f"appropriate and tailored to the patient's learning needs and health literacy level.",
            
            # Evaluation section
            f"\n\nEvaluation of Outcomes:\n"
            f"Ongoing evaluation of nursing interventions and patient outcomes is essential "
            f"for quality nursing care. Expected outcomes should be measurable, realistic, "
            f"and patient-centered, with modifications made as needed based on patient response."
        ]
        ...
        if concepts:
            ...
            for concept in concepts[:3]:  # Limit to 3 concepts for readability
                concept_section += f"• {concept.term} ({concept.umls_cui}): {concept.definition}\n"
                concept_section += f"  Clinical relevance: {concept.nursing_relevance}\n\n"
        ...
        if difficulty >= 4:
            content_sections.append(
                ...
                f"Advanced practice nurses should consider complex clinical decision-making, "
                f"interdisciplinary collaboration, and leadership in care coordination. "
                f"Critical thinking skills are essential for managing complex patient scenarios."
            )
    ...
    def _generate_learning_objectives(self, topic: str, difficulty: int) -> List[str]:
        ...
        base_objectives = [
            ...
            f"Implement evidence-based nursing interventions",
            f"Provide appropriate patient and family education",
            f"Evaluate effectiveness of nursing interventions"
        ]
...
class MedicalTestDatabaseManager:
    ...
    async def insert_medical_document(self, document: NursingEducationDocument, db_name: str = "bsn_knowledge_e2e"):
        ...
        async with self.engines[db_name].begin() as conn:
            ...
            result = await conn.execute(
                ...
                document.subject_area, document.nclex_category, document.difficulty_level,
                document.target_audience, document.umls_accuracy, document.content_hash,
                document.metadata.get("word_count", 0), document.metadata.get("concept_count", 0),
                document.learning_objectives, document.evidence_sources,
            ...
            for concept in document.medical_concepts:
                ...
                await conn.execute(
                    text(concept_sql),
                    doc_id, concept.term, concept.umls_cui, concept.semantic_type,
                    concept.definition, concept.synonyms, concept.category,
                    concept.confidence_score, concept.nursing_relevance,
                    concept.nclex_categories, concept.clinical_context
                )
            ...
            for scenario in document.clinical_scenarios:
                ...
                await conn.execute(
                    text(scenario_sql),
                    doc_id, scenario["scenario_id"], scenario["title"],
                    json.dumps(scenario["patient_context"]), 
                    scenario["learning_focus"], document.difficulty_level
                )
            ...
            await conn.execute(
                text(validation_sql),
                doc_id, "umls_terminology", document.umls_accuracy,
                "automated_umls_validation", "medical_test_generator",
        ...
        logger.info(f"Inserted document {document.document_id} with accuracy {document.umls_accuracy:.3f} in {processing_time:.2f}s")
        
        return doc_id
    
    async def record_performance_metric(self, run_id: str, metric_name: str, 
        ...
        for engine in self.engines.values():
            if "bsn_knowledge" in str(engine.url):
                async with engine.begin() as conn:
                    await conn.execute(
                        text(metric_sql), run_id, metric_name, metric_value, 
    ...
    async def get_generation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "accuracy_statistics": {
                "average_accuracy": self.performance_metrics.get("average_accuracy", 0.0),
                "documents_above_threshold": self.performance_metrics.get("documents_created", 0),
            ...
            "database_statistics": {
                ...
                "database_operations": self.performance_metrics.get("database_insertions", 0)
            },
            "processing_statistics": {
                "total_time_seconds": self.performance_metrics.get("total_processing_time", 0.0),
...
async def main():
    ...
    logger.info("Starting Group 1C: Test Data Preparation - Medical Content Test Database Creation")
    
    # Configuration
    config = TestDataGenerationConfig(
        total_documents=1000,
        min_medical_accuracy=0.98,
        target_umls_accuracy=0.995
    )
    ...
    generation_run_id = f"medical_data_gen_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        logger.info(f"Generating {config.total_documents} nursing education documents...")
        ...
        for domain in config.clinical_domains:
            domain_document_count = config.total_documents // len(config.clinical_domains)
            
            for difficulty in range(1, 6):
                difficulty_count = int(domain_document_count * config.difficulty_distribution[difficulty])
                ...
                for _ in range(difficulty_count):
                    ...
                    if document.umls_accuracy >= config.min_medical_accuracy:
                        ...
                        db_manager.performance_metrics["umls_validations"] += len(document.medical_concepts)
        ...
        final_accuracy = sum(accuracy_scores) / len(accuracy_scores) if accuracy_scores else 0.0
        ...
        await db_manager.record_performance_metric(
            generation_run_id, "average_processing_time", 
            sum(processing_times) / len(processing_times) if processing_times else 0.0, "seconds"
        )
        ...
        logger.info(f"Total Processing Time: {summary['processing_statistics']['total_time_seconds']:.2f}s")

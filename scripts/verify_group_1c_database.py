...
def verify_database():
    ...
    cursor.execute("SELECT subject_area, COUNT(*) FROM medical_documents GROUP BY subject_area")
    domain_distribution = cursor.fetchall()
    
    # Difficulty distribution
    cursor.execute("SELECT difficulty, COUNT(*) FROM medical_documents GROUP BY difficulty")  
    difficulty_distribution = cursor.fetchall()
    
    # Sample documents
    cursor.execute("SELECT title, subject_area, umls_accuracy FROM medical_documents LIMIT 5")

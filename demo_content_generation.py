...
class MockRAGnosticClient:
    ...
    async def validate_medical_content(self, content: str):
        ...
        return {
            ...
            "validation_notes": [
                "Content aligns with current evidence-based practices"
            ],
...
class MockOpenAIClient:
    ...
    async def generate_study_guide(self, topic: str) -> str:
        ...
        guide = {
            ...
            "sections": [
                {
                    ...
                    "learning_objectives": [
                        {
                            ...
                            "assessment_criteria": [
                                "Accurately defines terminology",
                    ...
                    "key_concepts": [
                        ...
                        f"{topic} assessment",
                        f"{topic} interventions",
                    ],
                    "clinical_applications": [
                        f"Acute {topic} management",
                        f"{topic} patient education",
                        f"Family support for {topic}",
                    ],
                    "study_questions": [
            ...
            "evidence_citations": [
                "Current Nursing Literature 2024",
        ...
        recommendations = {
            "recommendations": [
                ...
                },
                {
                    ...
                    "priority": "high",
                    "contraindications": [
                        "Known allergies to prescribed interventions"
                    ],
                    "monitoring_parameters": [
            ...
            "nursing_diagnoses": [
                f"Risk for complications related to {condition}",
                f"Knowledge deficit related to {condition} management",
            ],
...
class ContentGenerationDemo:
    ...
    async def demonstrate_nclex_generation(self, topic: str = "diabetes management"):
        ...
        logger.info(
            f"✅ Retrieved {len(context['items'])} content items with UMLS concepts"
        )

        # Generate NCLEX questions
        logger.info("🤖 Generating NCLEX questions using OpenAI...")
        questions_json = await self.openai_client.generate_nclex_questions(
            topic, count=2
        )

        # Validate medical accuracy
        logger.info("🔍 Validating medical accuracy...")
        validation = await self.ragnostic_client.validate_medical_content(
            questions_json
        )
        logger.info(
            f"✅ Medical accuracy validation: {validation['confidence_score']:.1%}"
        )
        ...
        for i, question in enumerate(questions, 1):
            ...
            logger.info(
                f"   Clinical Scenario: {question['clinical_scenario'][:100]}..."
            )
            logger.info(
                f"   Evidence Citations: {', '.join(question['evidence_citations'])}"
            )
    ...
    async def demonstrate_study_guide_generation(self, topic: str = "cardiac care"):
        ...
        logger.info(
            f"   Competency Frameworks: {', '.join(guide['competency_alignment'].keys())}"
        )
        ...
        for section in guide["sections"][:1]:  # Show first section
            ...
            logger.info(
                f"      Learning Objectives: {len(section['learning_objectives'])}"
            )
    ...
    async def demonstrate_clinical_support(self, condition: str = "heart failure"):
        ...
        recommendations_json = (
            await self.openai_client.generate_clinical_recommendations(condition)
        )
        ...
        logger.info(
            f"✅ Clinical accuracy validation: {validation['confidence_score']:.1%}"
        )
        ...
        logger.info(
            f"   Total Recommendations: {recommendations['evidence_summary']['total_recommendations']}"
        )
        logger.info(
            f"   High Evidence Count: {recommendations['evidence_summary']['high_evidence_count']}"
        )
        ...
        for i, rec in enumerate(recommendations["recommendations"][:2], 1):
            ...
            logger.info(
                f"      Evidence Level: {rec['evidence_level'].replace('_', ' ').title()}"
            )
        ...
        logger.info(
            f"   ⚠️ Safety Considerations: {', '.join(recommendations['safety_considerations'])}"
        )
    ...
    async def demonstrate_integration_workflow(self):
        ...
        results["study_guide"] = await self.demonstrate_study_guide_generation(
        ...
        logger.info(
            f"✅ Study Guide Sections: {len(results['study_guide']['guide']['sections'])}"
        )
        logger.info(
            f"✅ Clinical Recommendations: {len(results['clinical']['recommendations']['recommendations'])}"
        )
        logger.info(
            f"✅ Average Medical Accuracy: {((results['nclex']['validation']['confidence_score'] + results['study_guide']['validation']['confidence_score'] + results['clinical']['validation']['confidence_score']) / 3):.1%}"
        )
        logger.info(f"✅ Total OpenAI API Calls: {self.openai_client.call_count}")
        logger.info("✅ All validations passed medical accuracy threshold (>95%)")

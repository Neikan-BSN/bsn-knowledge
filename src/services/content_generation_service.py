...
class ContentGenerationService:
    ...
    async def _get_enriched_context(
        self, topic: str, filters: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            search_results = await self.ragnostic.search_content(
                query=topic, filters=filters, limit=5
            )
    ...
    async def _validate_medical_accuracy(
        self, content: str, topic: str, threshold: float = 0.95
        ...
        try:
            ...
            search_results = await self.ragnostic.search_content(
                query=f"evidence validation {topic}", limit=3
            )
            ...
            if confidence < threshold:
                issues.append(
                    f"Medical accuracy confidence {confidence:.2f} below threshold {threshold}"
                )
                is_accurate = False
    ...
    async def _generate_with_openai(
        self, system_prompt: str, user_prompt: str, response_format: str | None = None
    ) -> str:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning(
                    f"Rate limit exceeded, waiting {wait_time}s before retry {attempt + 1}"
                )
                ...
                if attempt == max_retries - 1:
                    logger.error(
                        f"OpenAI generation failed after {max_retries} attempts: {str(e)}"
                    )
    ...
    async def generate_content_with_validation(
        ...
        context = await self._get_enriched_context(
            request.topic, request.context_filters
        )
        ...
        for attempt in range(max_validation_attempts):
            try:
                ...
                if validation.is_accurate:
                    logger.info(
                        f"Content generated and validated successfully for {request.topic}"
                    )
                    return {
                        "content": generated_content,
                ...
                logger.error(
                    f"Content generation attempt {attempt + 1} failed: {str(e)}"
                )
                if attempt == max_validation_attempts - 1:
                    raise

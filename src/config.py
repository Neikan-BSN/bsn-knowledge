...
class Settings(BaseSettings):
    ...
    openai_max_tokens: int = Field(default=2000, env="OPENAI_MAX_TOKENS", ge=1, le=8000)
    
    # RAGnostic Configuration - Enhanced
    ragnostic_base_url: str = Field(default="http://localhost:8000", env="RAGNOSTIC_BASE_URL")
    ...
    ragnostic_connection_pool_size: int = Field(default=100, env="RAGNOSTIC_CONNECTION_POOL_SIZE")
    ragnostic_circuit_breaker_failure_threshold: int = Field(default=5, env="RAGNOSTIC_CIRCUIT_BREAKER_FAILURE_THRESHOLD")
    ragnostic_circuit_breaker_reset_timeout: int = Field(default=60, env="RAGNOSTIC_CIRCUIT_BREAKER_RESET_TIMEOUT")
    
    # Content Generation Settings
    medical_accuracy_threshold: float = Field(default=0.95, env="MEDICAL_ACCURACY_THRESHOLD", ge=0.8, le=1.0)
    max_validation_attempts: int = Field(default=3, env="MAX_VALIDATION_ATTEMPTS", ge=1, le=10)
    content_generation_timeout: int = Field(default=120, env="CONTENT_GENERATION_TIMEOUT")
    ...
    api_slow_request_threshold: float = Field(default=0.5, env="API_SLOW_REQUEST_THRESHOLD")

...
async def assess_competency_endpoint(
    request: CompetencyAssessmentRequest,
    framework: AACNCompetencyFramework = Depends(get_competency_framework_dep),
):
    """
    REVISED_PHASE3_PLAN.md Required Endpoint: Nursing competency assessment

    This is the specific endpoint required by Phase 3 planning documents.
    Delegates to the existing competency assessment functionality.
    """
    try:
        return await assess_competency(request, framework)
...
async def assess_competency(
    ...
    try:
        logger.info(
            f"Assessing competency {request.competency_id} for student {request.student_id}"
        )
        ...
        logger.info(
            f"Competency assessment completed: {assessment_result.current_level} level"
        )
...
async def assess_competencies_bulk(
    ...
    try:
        logger.info(
            f"Processing bulk assessment with {len(request.assessments)} assessments"
        )
        ...
        logger.info(
            f"Bulk assessment completed: {len(results)}/{len(request.assessments)} successful"
        )
        return response
...
async def get_student_competency_profile(
    ...
    include_historical: bool = Query(
        False, description="Include historical assessment data"
    ),
    framework: AACNCompetencyFramework = Depends(get_competency_framework_dep),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve competency profile"
        ) from e
        ...
        if request.severity_filter:
            ...
            for domain, gaps in gaps_by_domain.items():
                filtered_gaps[domain] = [
                    gap for gap in gaps if gap.severity == request.severity_filter
                ]
            gaps_by_domain = filtered_gaps

        total_gaps = sum(len(gaps) for gaps in gaps_by_domain.values())
        logger.info(
            f"Gap analysis completed: {total_gaps} gaps across {len(gaps_by_domain)} domains"
        )
...
async def generate_learning_path(
    ...
    try:
        ...
        logger.info(
            f"Learning path generated: {learning_path.estimated_duration_hours} hours, {len(learning_path.recommended_sequence)} activities"
        )
...
async def get_available_competencies(
    ...
    try:
        ...
        if domain:
            ...
            logger.info(
                f"Retrieved {len(competencies)} competencies for domain {domain}"
            )
        else:
...
async def get_aacn_domains():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve domains") from e
